import { Helper, Point3D, Draw, Color } from '../core';
...
export class Mesh {
  ...
  drawVertices() {
    ...
    const col = new Color(Draw.color);
    const rangeMin = -1500;
    const rangeMax = -1000;
    ...
    for (let i = 0; i < l; i++) {
      ...
      const nz = ((Math.max(rangeMin, Math.min(rangeMax, this.vertices[i].cameraTransformed.z))) - rangeMin) / range;
